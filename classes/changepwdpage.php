...
class ChangePasswordPage extends RunnerPage
{
	...
	protected function getSQLWhere()
	{
		...
		if( $this->token )
			return " where".$this->connection->addFieldWrappers( "token" )."=". $this->connection->prepareString( $this->token );
	...
	protected function getUpdateSQL( $newpass, $where )
	{
		...
		if( $this->token )
		{
			$setPart.= ", ".$this->connection->addFieldWrappers( "token" )."=". $this->connection->prepareString( "" ).", "
				.$this->connection->addFieldWrappers( "date_created" )."=". $this->connection->addDateQuotes( NULL );
	...
	protected function checkToken()
	{
		...
		$sqlSelect = "select ".$this->connection->addFieldWrappers( "date_created" )." from".$this->connection->addTableWrappers("users")
			." where".$this->connection->addFieldWrappers( "token" )."=". $this->connection->prepareString( $this->token );
			
		$data = $this->cipherer->DecryptFetchedArray( $this->connection->query( $sqlSelect )->fetchAssoc() );
		if( $data )
			return secondsPassedFrom( $data["date_created"] ) < 86400;
