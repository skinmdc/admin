...
class RemindPasswordPage extends RunnerPage
{
	...
	{		
		...
		if( !$tosearch || !$data )
		{
			$this->message = "User"." ".runner_htmlspecialchars($this->userEmail)." "."is not registered.";
			return false;
		}
		...
		if( $this->remindMethod() == RPM_RESET )
		{
			...
			$this->connection->exec("update ".$this->connection->addTableWrappers("users")." set "
				.$this->connection->addFieldWrappers( "token" )."=". $this->connection->prepareString( $token ).","
				.$this->connection->addFieldWrappers( "date_created" )."=". $this->connection->addDateQuotes( now() ).""
		...
		else
		{
				if( !$this->cipherer->isFieldEncrypted( $this->passwordFiled ) )
			{
				...
				$this->connection->exec("update ".$this->connection->addTableWrappers("users")
 ...
 //RPM_RESET
	...
	protected function getUserSearchingSQLData()
	{
		...
		{
			$fullcUserNameField = $this->connection->addTableWrappers( "users" ).".".$this->connection->addFieldWrappers( $this->usernameFiled );
			$fullcEmailField = $this->connection->addTableWrappers( "users" ).".".$this->connection->addFieldWrappers( $this->emailField );
			$fullcPasswordField = $this->connection->addTableWrappers( "users" ).".".$this->connection->addFieldWrappers( $this->passwordFiled );
		}
	...
	function assignBodyEnd($params = "") 
	{
		...
		echo "<script language=\"JavaScript\" src=\""
			.GetRootPathForResources("include/runnerJS/RunnerAll.js?33978")."\"></script>\r\n";
