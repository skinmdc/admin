...
function checkTableName($shortTName, $type=false)
{
	...
	if ("users" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("services" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("doctors" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("accounts" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("appointments" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("administrator" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("new_customer" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("scheduled_appointments" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("pending_appointments" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("cancelled_appointments" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("appointments___reschedule" == $shortTName && ($type===false || ($type!==false && $type == 4)))
		return true;
	if ("walkin" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("new_walkin" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("scheduled" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("products" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("product_orders" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("doctors");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("accounts");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("appointments");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Administrator");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("New Customer");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Scheduled Appointments");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="Scheduled Appointments";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Pending Appointments");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="Pending Appointments";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Cancelled Appointments");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="Cancelled Appointments";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Appointments / Reschedule");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="Appointments / Reschedule";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("walkin");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("new_walkin");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("scheduled");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("product_orders");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
...
function GetUserPermissionsStatic( $table )
{
	...
	if( $table=="users" )
	{
		if( $sUserGroup=="Administrator" )
		{
			return "AEDSPI".$extraPerm;
		}
		if( $sUserGroup=="Customer Account" )
		{
			return "".$extraPerm;
		}
		if( $sUserGroup=="Deactivated" )
		{
			return "".$extraPerm;
		}
		if( $sUserGroup=="Doctor Account" )
		{
			return "".$extraPerm;
		}
//	default permissions
...
//	default permissions
	...
	if( $table=="Scheduled Appointments" )
	{
		if( $sUserGroup=="Administrator" )
		{
			return "AEDSPI".$extraPerm;
...
//	default permissions
	...
	if( $table=="Cancelled Appointments" )
	{
		if( $sUserGroup=="Administrator" )
		{
			return "AEDSPI".$extraPerm;
...
//	default permissions
	...
	if( $table=="Appointments / Reschedule" )
	{
		if( $sUserGroup=="Administrator" )
		{
			return "S".$extraPerm;
...
function SetAuthSessionData($pUsername, &$data, $password, &$pageObject = null, $fireEvents = true )
{
	...
	if( Security::permissionsAvailable() ) {
		...
		$_SESSION["OwnerID"] = $data["access_type"];
	$_SESSION["_users_OwnerID"] = $data["access_type"];
...
{
	$result = "";
	$result .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/loadfirst.js?33978")."\"></script>";
	$result .= $additionalScripts;
	$result .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/lang/".getLangFileName(mlang_getcurrentlang()).".js?33978")."\"></script>";
